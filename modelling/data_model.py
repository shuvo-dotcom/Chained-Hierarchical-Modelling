...
class Data():
    def __init__(self,
        ...
        y_encoded = self.label_encoder.fit_transform(y_good)

        new_test_size = X.shape[0] * 0.2 / X_good.shape[0]

        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            X_good, y_encoded, test_size=new_test_size, random_state=0, stratify=y_encoded)
        self.y = y_encoded
    ...
    def decode_predictions(self, predictions: np.ndarray) -> np.ndarray:
        ...
        :param predictions: Encoded predictions
